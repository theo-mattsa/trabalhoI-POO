...
int main(int argc, char const* argv[]) {
  ...
  if (argc != 5) {
    cout << "Use o formato: ./vereadores <codigo_cidade> <arquivo_candidatos> <arquivo_votacao> <data>" << endl;
    return 1;
  }

  // Obtem os dados providos pelos argumentos
  int codigoCidade = stoi(argv[1]);
  string arquivoCandidatos = argv[2];
  string arquivoVotacao = argv[3];
  string dataEleicao = argv[4];

  // Cria eleicao e processa os dados
