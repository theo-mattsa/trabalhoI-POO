...
TEST_DIR=testes
TEST_SCRIPT=test.sh
BASE_DIR=$TEMP_DIR/progoo-tests-$(whoami)
SUBDIR_INPUT=in
SUBDIR_OUTPUT=out
	...
	if [ -d $dir ] && [ "$dir" != "$TEST_DIR" ]; then
		...
		if [ $exitcode = 0 ]; then
			...
			for subdir in `ls $TEST_DIR`; do
				...
				# Runs the test.
				$TEST_DIR/$subdir/$TEST_SCRIPT "$subdir" "$dir" "$srcdir" "$TEMP_DIR" "$BASE_DIR" "$TEST_DIR/$subdir" "$SUBDIR_INPUT" "$SUBDIR_OUTPUT" "diff -d --color=auto -y --strip-trailing-cr --suppress-common-lines --ignore-all-space --ignore-blank-lines -W 200"
...
echo "Script de teste PROG OO - Trabalho 2"
echo
